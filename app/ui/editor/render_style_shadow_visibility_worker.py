# -*- coding: utf-8 -*-
"""One-shot Qt worker for exact selected-parent shadow-visibility commands."""
...
def _exact_page(project: Mapping[str, Any], page_id: str) -> Mapping[str, Any]:
    ...
    if not isinstance(pages, list):
        raise RenderStyleShadowVisibilityCommandError(
            RenderStyleShadowVisibilityCommandErrorCode.PAGE_NOT_FOUND,
...
class RenderStyleShadowVisibilityWorker(QtCore.QObject):
    """Commit one canonical shadow-visibility command in a dedicated thread."""
    ...
    def __init__(self, command: RenderStyleShadowVisibilityWorkerCommand) -> None:
        super().__init__()
    ...
    def run(self) -> None:
        if not self._claim_run():
            self.failure.emit(
                self._failure(
                    RenderStyleShadowVisibilityWorkerFailureCode.WORKER_REUSED,
                    RenderStyleShadowVisibilityWorkerStage.LOADING_PROJECT,
                    "RenderStyleShadowVisibilityWorker instances are one-shot.",
                )
            )
            self.finished.emit()
            return
        ...
        core_receipt: RenderStyleShadowVisibilityCommandReceipt | None = None
        terminal_receipt: RenderStyleShadowVisibilityWorkerReceipt | None = None
        terminal_failure: RenderStyleShadowVisibilityWorkerFailure | None = None
        terminal_cancelled: RenderStyleShadowVisibilityCancelledReceipt | None = None
        stage = RenderStyleShadowVisibilityWorkerStage.LOADING_PROJECT
        self._emit_cancellation(
            "Shadow visibility can be cancelled before persistence."
        )
        ...
        try:
            ...
            stage = RenderStyleShadowVisibilityWorkerStage.OPENING_EDIT_STORE
            ...
            if store is None:
                ...
                stage = RenderStyleShadowVisibilityWorkerStage.READING_SNAPSHOT
            ...
            if store is None:
                stage = RenderStyleShadowVisibilityWorkerStage.PROJECTING
                ...
                if len(parent_matches) != 1:
                    raise RenderStyleShadowVisibilityCommandError(
                        RenderStyleShadowVisibilityCommandErrorCode.PARENT_NOT_FOUND,
                        "The selected parent is no longer available.",
                    )
                self._cancel_checkpoint(stage)

            stage = RenderStyleShadowVisibilityWorkerStage.PREPARING_COMMAND
            self._emit_busy(stage, "Preparing the typed shadow-visibility edit...")
            command = RenderStyleShadowVisibilityCommand(
                ...
                shadow_enabled=self.command.shadow_enabled,
            ...
            stage = RenderStyleShadowVisibilityWorkerStage.PERSISTING
            self._emit_cancellation(
                "The shadow-visibility edit is being persisted and can no longer be cancelled."
            )
            self._emit_busy(stage, "Saving the shadow-visibility edit...")
            if store is None:
                store = _open_project_edit_store(
                    self.command.project_path,
                    project,
            ...
            core_receipt = RenderStyleShadowVisibilityCommandService(
                edit_store=store
            ).execute(
                project=project,
                command=command,
            ...
            stage = RenderStyleShadowVisibilityWorkerStage.MATERIALIZING_PROJECT
            ...
            stage = RenderStyleShadowVisibilityWorkerStage.BUILDING_UI_PROJECTION
            ...
            terminal_receipt = RenderStyleShadowVisibilityWorkerReceipt(
                command_receipt=core_receipt,
                project=materialized,
                projection=projection,
            )
        except _CancelledBeforePersistence:
            terminal_cancelled = RenderStyleShadowVisibilityCancelledReceipt(
                project_path=self.command.project_path,
                page_id=self.command.page_id,
                parent_id=self.command.parent_id,
                operation=self.command.operation,
        ...
        except RenderStyleShadowVisibilityCommandError as exc:
            terminal_failure = self._failure_from_command(stage, exc)
        except FileNotFoundError as exc:
            terminal_failure = self._failure(
                RenderStyleShadowVisibilityWorkerFailureCode.PROJECT_LOAD_FAILED,
                stage,
            ...
            terminal_failure = self._failure(
                ...
                stage,
                str(exc) or "The shadow-visibility command is invalid.",
                exc,
                core_receipt=core_receipt,
            )
            ...
            terminal_failure = self._failure(
                self._failure_code_for_stage(stage, core_receipt is not None),
                stage,
                str(exc) or "Shadow-visibility I/O failed.",
                exc,
                core_receipt=core_receipt,
            )
            ...
            terminal_failure = self._failure(
                self._failure_code_for_stage(stage, core_receipt is not None),
                stage,
                str(exc) or "Shadow-visibility edit failed.",
                exc,
                core_receipt=core_receipt,
            )
        finally:
            if store is not None:
                try:
                    stage = RenderStyleShadowVisibilityWorkerStage.CLOSING_EDIT_STORE
                    ...
                    terminal_failure = self._failure(
                        RenderStyleShadowVisibilityWorkerFailureCode.EDIT_STORE_FAILED,
                        stage,
        ...
        self._emit_cancellation("Shadow-visibility worker finished.")
        self.busy.emit(
            RenderStyleShadowVisibilityWorkerBusyState(
                page_id=self.command.page_id,
                parent_id=self.command.parent_id,
                ...
                message="Shadow-visibility worker finished.",
            )
        )
        if terminal_receipt is not None:
            ...
            self.failure.emit(
                self._failure(
                    RenderStyleShadowVisibilityWorkerFailureCode.COMMAND_REJECTED,
                    stage,
                    "Shadow-visibility worker ended without a typed result.",
                    core_receipt=core_receipt,
                )
            )
        self.finished.emit()
    ...
    def _failure_from_command(
        self,
        stage: RenderStyleShadowVisibilityWorkerStage,
        exc: RenderStyleShadowVisibilityCommandError,
    ) -> RenderStyleShadowVisibilityWorkerFailure:
        code = {
            RenderStyleShadowVisibilityCommandErrorCode.PAGE_NOT_FOUND: RenderStyleShadowVisibilityWorkerFailureCode.PAGE_NOT_FOUND,
            RenderStyleShadowVisibilityCommandErrorCode.PARENT_NOT_FOUND: RenderStyleShadowVisibilityWorkerFailureCode.PARENT_NOT_FOUND,
            RenderStyleShadowVisibilityCommandErrorCode.PARENT_EXCLUDED: RenderStyleShadowVisibilityWorkerFailureCode.PARENT_UNAVAILABLE,
            RenderStyleShadowVisibilityCommandErrorCode.PARENT_NOT_RENDER_REQUIRED: RenderStyleShadowVisibilityWorkerFailureCode.PARENT_UNAVAILABLE,
            RenderStyleShadowVisibilityCommandErrorCode.AUTOMATIC_SHADOW_UNAVAILABLE: RenderStyleShadowVisibilityWorkerFailureCode.AUTOMATIC_SHADOW_UNAVAILABLE,
            RenderStyleShadowVisibilityCommandErrorCode.NO_OP: RenderStyleShadowVisibilityWorkerFailureCode.NO_OP,
            RenderStyleShadowVisibilityCommandErrorCode.STALE_EFFECTIVE_PAGE: RenderStyleShadowVisibilityWorkerFailureCode.SNAPSHOT_STALE,
            RenderStyleShadowVisibilityCommandErrorCode.STALE_PAGE_HEAD: RenderStyleShadowVisibilityWorkerFailureCode.SNAPSHOT_STALE,
            RenderStyleShadowVisibilityCommandErrorCode.STALE_GLOBAL_HEAD: RenderStyleShadowVisibilityWorkerFailureCode.SNAPSHOT_STALE,
            RenderStyleShadowVisibilityCommandErrorCode.SHADOW_VISIBILITY_SLOT_CONFLICT: RenderStyleShadowVisibilityWorkerFailureCode.SHADOW_VISIBILITY_SLOT_CONFLICT,
            RenderStyleShadowVisibilityCommandErrorCode.DUPLICATE_COMMAND: RenderStyleShadowVisibilityWorkerFailureCode.DUPLICATE_COMMAND,
            RenderStyleShadowVisibilityCommandErrorCode.PROJECTION_REJECTED: RenderStyleShadowVisibilityWorkerFailureCode.PROJECTION_FAILED,
            RenderStyleShadowVisibilityCommandErrorCode.PROJECT_IDENTITY_MISMATCH: RenderStyleShadowVisibilityWorkerFailureCode.PROJECT_INVALID,
            RenderStyleShadowVisibilityCommandErrorCode.STORE_IDENTITY_MISMATCH: RenderStyleShadowVisibilityWorkerFailureCode.EDIT_STORE_FAILED,
        }.get(
            exc.code,
            RenderStyleShadowVisibilityWorkerFailureCode.COMMAND_REJECTED,
        )
        return self._failure(
            code,
            stage,
            str(exc) or "Shadow-visibility command was rejected.",
            exc,
    ...
    def _failure(
        ...
        *,
        ...
        if committed and code not in {
            RenderStyleShadowVisibilityWorkerFailureCode.EDIT_STORE_FAILED,
            RenderStyleShadowVisibilityWorkerFailureCode.POST_COMMIT_PROJECTION_FAILED,
        }:
            code = (
                RenderStyleShadowVisibilityWorkerFailureCode.POST_COMMIT_PROJECTION_FAILED
            )
        return RenderStyleShadowVisibilityWorkerFailure(
            code=code,
            stage=stage,
            project_path=self.command.project_path,
            page_id=self.command.page_id,
            ...
            operation=self.command.operation,
            shadow_enabled=self.command.shadow_enabled,
            message=str(message or "Shadow-visibility edit failed."),
    ...
    def _failure_code_for_stage(
        ...
        if stage is RenderStyleShadowVisibilityWorkerStage.LOADING_PROJECT:
            return RenderStyleShadowVisibilityWorkerFailureCode.PROJECT_LOAD_FAILED
        if stage in {
            RenderStyleShadowVisibilityWorkerStage.OPENING_EDIT_STORE,
            RenderStyleShadowVisibilityWorkerStage.READING_SNAPSHOT,
            RenderStyleShadowVisibilityWorkerStage.PERSISTING,
            RenderStyleShadowVisibilityWorkerStage.CLOSING_EDIT_STORE,
        }:
            return RenderStyleShadowVisibilityWorkerFailureCode.EDIT_STORE_FAILED
        if stage in {
            RenderStyleShadowVisibilityWorkerStage.PROJECTING,
            RenderStyleShadowVisibilityWorkerStage.BUILDING_UI_PROJECTION,
        }:
            return RenderStyleShadowVisibilityWorkerFailureCode.PROJECTION_FAILED
        return RenderStyleShadowVisibilityWorkerFailureCode.PROJECT_INVALID


__all__ = ["RenderStyleShadowVisibilityWorker"]
