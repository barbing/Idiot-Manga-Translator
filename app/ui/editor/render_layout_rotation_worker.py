# -*- coding: utf-8 -*-
"""One-shot Qt worker for exact selected-parent rotation commands."""
...
class RenderLayoutRotationWorker(QtCore.QObject):
    """Commit one canonical rotation command in a dedicated thread."""
    ...
    def __init__(self, command: RenderLayoutRotationWorkerCommand) -> None:
        super().__init__()
    ...
    def run(self) -> None:
        if not self._claim_run():
            self.failure.emit(
                self._failure(
                    RenderLayoutRotationWorkerFailureCode.WORKER_REUSED,
                    RenderLayoutRotationWorkerStage.LOADING_PROJECT,
                    "RenderLayoutRotationWorker instances are one-shot.",
                )
            )
            self.finished.emit()
            return
        ...
        core_receipt: RenderLayoutRotationCommandReceipt | None = None
        terminal_receipt: RenderLayoutRotationWorkerReceipt | None = None
        terminal_failure: RenderLayoutRotationWorkerFailure | None = None
        terminal_cancelled: RenderLayoutRotationCancelledReceipt | None = None
        stage = RenderLayoutRotationWorkerStage.LOADING_PROJECT
        self._emit_cancellation(
            "Rotation can be cancelled before persistence."
        )
        ...
        try:
            ...
            stage = RenderLayoutRotationWorkerStage.OPENING_EDIT_STORE
            ...
            if store is None:
                ...
                stage = RenderLayoutRotationWorkerStage.READING_SNAPSHOT
            ...
            stage = RenderLayoutRotationWorkerStage.PROJECTING
            ...
            self._emit_cancellation(
                "The rotation edit is being persisted and can no longer be cancelled."
            )
            if store is None:
                stage = RenderLayoutRotationWorkerStage.OPENING_EDIT_STORE
            ...
            stage = RenderLayoutRotationWorkerStage.PREPARING_COMMAND
            self._emit_busy(stage, "Preparing the typed rotation edit...")
            command = RenderLayoutRotationCommand(
                ...
                rotation=self.command.rotation,
            ...
            stage = RenderLayoutRotationWorkerStage.PERSISTING
            self._emit_busy(stage, "Saving the rotation edit...")
            core_receipt = RenderLayoutRotationCommandService(
                edit_store=store
            ...
            stage = RenderLayoutRotationWorkerStage.MATERIALIZING_PROJECT
            ...
            stage = RenderLayoutRotationWorkerStage.BUILDING_UI_PROJECTION
            ...
            terminal_receipt = RenderLayoutRotationWorkerReceipt(
                command_receipt=core_receipt,
                project=materialized,
                projection=projection,
            )
        except _CancelledBeforePersistence:
            terminal_cancelled = RenderLayoutRotationCancelledReceipt(
                project_path=self.command.project_path,
                page_id=self.command.page_id,
                parent_id=self.command.parent_id,
                operation=self.command.operation,
        ...
        except RenderLayoutRotationCommandError as exc:
            terminal_failure = self._failure_from_command(stage, exc)
        except FileNotFoundError as exc:
            terminal_failure = self._failure(
                RenderLayoutRotationWorkerFailureCode.PROJECT_LOAD_FAILED,
                stage,
            ...
            terminal_failure = self._failure(
                ...
                stage,
                str(exc) or "The rotation command is invalid.",
                exc,
                core_receipt=core_receipt,
            )
            ...
            terminal_failure = self._failure(
                self._failure_code_for_stage(stage, core_receipt is not None),
                stage,
                str(exc) or "Rotation I/O failed.",
                exc,
                core_receipt=core_receipt,
            )
            ...
            terminal_failure = self._failure(
                self._failure_code_for_stage(stage, core_receipt is not None),
                stage,
                str(exc) or "Rotation edit failed.",
                exc,
                core_receipt=core_receipt,
            )
        finally:
            if store is not None:
                try:
                    stage = RenderLayoutRotationWorkerStage.CLOSING_EDIT_STORE
                    ...
                    terminal_failure = self._failure(
                        RenderLayoutRotationWorkerFailureCode.EDIT_STORE_FAILED,
                        stage,
        ...
        self._emit_cancellation("Rotation worker finished.")
        self.busy.emit(
            RenderLayoutRotationWorkerBusyState(
                page_id=self.command.page_id,
                parent_id=self.command.parent_id,
                ...
                message="Rotation worker finished.",
            )
        )
        if terminal_receipt is not None:
            ...
            self.failure.emit(
                self._failure(
                    RenderLayoutRotationWorkerFailureCode.COMMAND_REJECTED,
                    stage,
                    "Rotation worker ended without a typed result.",
                    core_receipt=core_receipt,
                )
            )
        self.finished.emit()
    ...
    def _failure_from_command(
        self,
        stage: RenderLayoutRotationWorkerStage,
        exc: RenderLayoutRotationCommandError,
    ) -> RenderLayoutRotationWorkerFailure:
        code = {
            RenderLayoutRotationCommandErrorCode.PAGE_NOT_FOUND: RenderLayoutRotationWorkerFailureCode.PAGE_NOT_FOUND,
            RenderLayoutRotationCommandErrorCode.PARENT_NOT_FOUND: RenderLayoutRotationWorkerFailureCode.PARENT_NOT_FOUND,
            RenderLayoutRotationCommandErrorCode.PARENT_EXCLUDED: RenderLayoutRotationWorkerFailureCode.PARENT_UNAVAILABLE,
            RenderLayoutRotationCommandErrorCode.PARENT_NOT_RENDER_REQUIRED: RenderLayoutRotationWorkerFailureCode.PARENT_UNAVAILABLE,
            RenderLayoutRotationCommandErrorCode.AUTOMATIC_ROTATION_UNAVAILABLE: RenderLayoutRotationWorkerFailureCode.AUTOMATIC_ROTATION_UNAVAILABLE,
            RenderLayoutRotationCommandErrorCode.NO_OP: RenderLayoutRotationWorkerFailureCode.NO_OP,
            RenderLayoutRotationCommandErrorCode.STALE_EFFECTIVE_PAGE: RenderLayoutRotationWorkerFailureCode.SNAPSHOT_STALE,
            RenderLayoutRotationCommandErrorCode.STALE_PAGE_HEAD: RenderLayoutRotationWorkerFailureCode.SNAPSHOT_STALE,
            RenderLayoutRotationCommandErrorCode.STALE_GLOBAL_HEAD: RenderLayoutRotationWorkerFailureCode.SNAPSHOT_STALE,
            RenderLayoutRotationCommandErrorCode.ROTATION_SLOT_CONFLICT: RenderLayoutRotationWorkerFailureCode.ROTATION_SLOT_CONFLICT,
            RenderLayoutRotationCommandErrorCode.DUPLICATE_COMMAND: RenderLayoutRotationWorkerFailureCode.DUPLICATE_COMMAND,
            RenderLayoutRotationCommandErrorCode.PROJECTION_REJECTED: RenderLayoutRotationWorkerFailureCode.PROJECTION_FAILED,
            RenderLayoutRotationCommandErrorCode.PROJECT_IDENTITY_MISMATCH: RenderLayoutRotationWorkerFailureCode.PROJECT_INVALID,
            RenderLayoutRotationCommandErrorCode.STORE_IDENTITY_MISMATCH: RenderLayoutRotationWorkerFailureCode.EDIT_STORE_FAILED,
        }.get(
            exc.code,
            RenderLayoutRotationWorkerFailureCode.COMMAND_REJECTED,
        )
        return self._failure(
            code,
            stage,
            str(exc) or "Rotation command was rejected.",
            exc,
    ...
    def _failure(
        ...
        *,
        ...
        if committed and code not in {
            RenderLayoutRotationWorkerFailureCode.EDIT_STORE_FAILED,
            RenderLayoutRotationWorkerFailureCode.POST_COMMIT_PROJECTION_FAILED,
        }:
            code = (
                RenderLayoutRotationWorkerFailureCode.POST_COMMIT_PROJECTION_FAILED
            )
        return RenderLayoutRotationWorkerFailure(
            code=code,
            stage=stage,
            project_path=self.command.project_path,
            page_id=self.command.page_id,
            ...
            operation=self.command.operation,
            rotation=self.command.rotation,
            message=str(message or "Rotation edit failed."),
    ...
    def _failure_code_for_stage(
        ...
        if stage is RenderLayoutRotationWorkerStage.LOADING_PROJECT:
            return RenderLayoutRotationWorkerFailureCode.PROJECT_LOAD_FAILED
        if stage in {
            RenderLayoutRotationWorkerStage.OPENING_EDIT_STORE,
            RenderLayoutRotationWorkerStage.READING_SNAPSHOT,
            RenderLayoutRotationWorkerStage.PERSISTING,
            RenderLayoutRotationWorkerStage.CLOSING_EDIT_STORE,
        }:
            return RenderLayoutRotationWorkerFailureCode.EDIT_STORE_FAILED
        if stage in {
            RenderLayoutRotationWorkerStage.PROJECTING,
            RenderLayoutRotationWorkerStage.BUILDING_UI_PROJECTION,
        }:
            return RenderLayoutRotationWorkerFailureCode.PROJECTION_FAILED
        return RenderLayoutRotationWorkerFailureCode.PROJECT_INVALID


__all__ = ["RenderLayoutRotationWorker"]
