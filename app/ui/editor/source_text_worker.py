# -*- coding: utf-8 -*-
"""One-shot Qt worker for exact selected-parent source-text commands."""
...
class SourceTextWorker(QtCore.QObject):
    """Commit one source-text command inside a dedicated worker thread."""
    ...
    def __init__(self, command: SourceTextWorkerCommand) -> None:
        super().__init__()
    ...
    def run(self) -> None:
        if not self._claim_run():
            self.failure.emit(
                self._failure(
                    SourceTextWorkerFailureCode.WORKER_REUSED,
                    SourceTextWorkerStage.LOADING_PROJECT,
                    "SourceTextWorker instances are one-shot.",
                )
            )
            self.finished.emit()
            return
        ...
        core_receipt: SourceTextCommandReceipt | None = None
        terminal_receipt: SourceTextWorkerReceipt | None = None
        terminal_failure: SourceTextWorkerFailure | None = None
        terminal_cancelled: SourceTextCancelledReceipt | None = None
        stage = SourceTextWorkerStage.LOADING_PROJECT
        self._emit_cancellation(
            "Source text can be cancelled before persistence."
        )
        ...
        try:
            ...
            stage = SourceTextWorkerStage.OPENING_EDIT_STORE
            ...
            if store is None:
                ...
                stage = SourceTextWorkerStage.READING_SNAPSHOT
            ...
            stage = SourceTextWorkerStage.PROJECTING
            ...
            stage = SourceTextWorkerStage.PREPARING_COMMAND
            self._emit_busy(stage, "Preparing the typed source-text edit...")
            ...
            stage = SourceTextWorkerStage.PERSISTING
            self._emit_cancellation(
                "The source-text edit is being persisted and can no longer be cancelled."
            )
            self._emit_busy(stage, "Saving the source-text edit...")
            if store is None:
                store = _open_project_edit_store(
                    self.command.project_path,
                    project,
            ...
            core_receipt = SourceTextCommandService(edit_store=store).execute(
                project=project,
                command=command,
            )

            stage = SourceTextWorkerStage.MATERIALIZING_PROJECT
            ...
            stage = SourceTextWorkerStage.BUILDING_UI_PROJECTION
            ...
            terminal_receipt = SourceTextWorkerReceipt(
                command_receipt=core_receipt,
                project=materialized,
                projection=projection,
            )
        except _CancelledBeforePersistence:
            terminal_cancelled = SourceTextCancelledReceipt(
                project_path=self.command.project_path,
                page_id=self.command.page_id,
                parent_id=self.command.parent_id,
                operation=self.command.operation,
            ...
            terminal_failure = self._failure(
                SourceTextWorkerFailureCode.PROJECT_LOAD_FAILED,
                stage,
            ...
            terminal_failure = self._failure(
                ...
                stage,
                str(exc) or "The source-text command is invalid.",
                exc,
                core_receipt=core_receipt,
            )
            ...
            terminal_failure = self._failure(
                self._failure_code_for_stage(stage, core_receipt is not None),
                stage,
                str(exc) or "Source-text I/O failed.",
                exc,
                core_receipt=core_receipt,
            )
            ...
            terminal_failure = self._failure(
                self._failure_code_for_stage(stage, core_receipt is not None),
                stage,
                str(exc) or "Source-text edit failed.",
                exc,
                core_receipt=core_receipt,
            )
        finally:
            if store is not None:
                try:
                    stage = SourceTextWorkerStage.CLOSING_EDIT_STORE
                    ...
                    terminal_failure = self._failure(
                        SourceTextWorkerFailureCode.EDIT_STORE_FAILED,
                        stage,
        ...
        self._emit_cancellation("Source-text worker finished.")
        self.busy.emit(
            SourceTextWorkerBusyState(
                page_id=self.command.page_id,
                parent_id=self.command.parent_id,
                ...
                message="Source-text worker finished.",
            )
        )
        if terminal_receipt is not None:
            ...
            self.failure.emit(
                self._failure(
                    SourceTextWorkerFailureCode.COMMAND_REJECTED,
                    stage,
                    "Source-text worker ended without a typed result.",
                    core_receipt=core_receipt,
                )
            )
        self.finished.emit()

    def _cancel_checkpoint(self, stage: SourceTextWorkerStage) -> None:
        with self._cancel_lock:
    ...
    def _emit_busy(self, stage: SourceTextWorkerStage, message: str) -> None:
        with self._cancel_lock:
    ...
    def _failure_from_command(
        ...
        exc: SourceTextCommandError,
    ) -> SourceTextWorkerFailure:
        code = {
            SourceTextCommandErrorCode.PAGE_NOT_FOUND: SourceTextWorkerFailureCode.PAGE_NOT_FOUND,
            SourceTextCommandErrorCode.PARENT_NOT_FOUND: SourceTextWorkerFailureCode.PARENT_NOT_FOUND,
            SourceTextCommandErrorCode.STALE_EFFECTIVE_PAGE: SourceTextWorkerFailureCode.SNAPSHOT_STALE,
            SourceTextCommandErrorCode.STALE_PAGE_HEAD: SourceTextWorkerFailureCode.SNAPSHOT_STALE,
            SourceTextCommandErrorCode.STALE_GLOBAL_HEAD: SourceTextWorkerFailureCode.SNAPSHOT_STALE,
            SourceTextCommandErrorCode.SOURCE_SLOT_CONFLICT: SourceTextWorkerFailureCode.SOURCE_SLOT_CONFLICT,
            SourceTextCommandErrorCode.DUPLICATE_COMMAND: SourceTextWorkerFailureCode.DUPLICATE_COMMAND,
            SourceTextCommandErrorCode.PROJECTION_REJECTED: SourceTextWorkerFailureCode.PROJECTION_FAILED,
            SourceTextCommandErrorCode.PROJECT_IDENTITY_MISMATCH: SourceTextWorkerFailureCode.PROJECT_INVALID,
            SourceTextCommandErrorCode.STORE_IDENTITY_MISMATCH: SourceTextWorkerFailureCode.EDIT_STORE_FAILED,
        }.get(exc.code, SourceTextWorkerFailureCode.COMMAND_REJECTED)
        return self._failure(
            code,
            stage,
            str(exc) or "Source-text command was rejected.",
            exc,
    ...
    def _failure(
        ...
        *,
        ...
        if committed and code not in {
            SourceTextWorkerFailureCode.EDIT_STORE_FAILED,
            SourceTextWorkerFailureCode.POST_COMMIT_PROJECTION_FAILED,
        }:
            code = SourceTextWorkerFailureCode.POST_COMMIT_PROJECTION_FAILED
        return SourceTextWorkerFailure(
            code=code,
            stage=stage,
            project_path=self.command.project_path,
            page_id=self.command.page_id,
            ...
            operation=self.command.operation,
            text=self.command.text,
            message=str(message or "Source-text edit failed."),
    ...
    def _failure_code_for_stage(
        ...
        if committed:
            return SourceTextWorkerFailureCode.POST_COMMIT_PROJECTION_FAILED
        if stage is SourceTextWorkerStage.LOADING_PROJECT:
            return SourceTextWorkerFailureCode.PROJECT_LOAD_FAILED
        if stage in {
            SourceTextWorkerStage.OPENING_EDIT_STORE,
            SourceTextWorkerStage.READING_SNAPSHOT,
            SourceTextWorkerStage.PERSISTING,
            SourceTextWorkerStage.CLOSING_EDIT_STORE,
        }:
            return SourceTextWorkerFailureCode.EDIT_STORE_FAILED
        if stage in {
            SourceTextWorkerStage.PROJECTING,
            SourceTextWorkerStage.BUILDING_UI_PROJECTION,
        }:
            return SourceTextWorkerFailureCode.PROJECTION_FAILED
        return SourceTextWorkerFailureCode.PROJECT_INVALID


__all__ = ["SourceTextWorker"]
